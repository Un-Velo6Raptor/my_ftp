...
{
	int fd_file = open(file, O_CREAT | O_WRONLY | O_TRUNC);
	char buffer[1024];
	int nb_read = 0;
...
static int stor_pasv(t_client *client, char *file)
{
	...
	ret = read_socket(client, client->data_mng.fd_socket, file);
...
int command_stor(int fd_server __attribute__((unused)),
	t_client *client,
